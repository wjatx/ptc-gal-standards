...
| PTC-8 | Producer | The agent authors intent, not identity: it supplies `event_id`, target `principal`, `payload`; the broker sets `sender`, `provenance`, `ts`, `expiry` (broker authorship of these fields: not yet implemented — #15); `sender_class` is absent on the wire. | PUBLISH P4 |
